...
const recipeUpload = upload.fields([{ name: 'thumbnail', maxCount: 1 }, { name: 'video_file', maxCount: 1 }]);
  ...
  try {
    ...
    if (adminCheck.rows.length === 0) {
      ...
      await client.query(
        ...
        ['Admin', 'admin@recipehub.com', hashedPw, '9999999999', 'Other', 30, 'admin']
      );
...
app.get('/api/profile', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query('SELECT id, name, email, mobile, gender, age, profile_photo, role, created_at FROM users WHERE id = $1', [req.user.id]);
...
// Update profile
app.put('/api/profile', authMiddleware, upload.single('profile_photo'), async (req, res) => {
  try {
...
app.post('/api/recipes', authMiddleware, recipeUpload, async (req, res) => {
  try {
    const { title, description, ingredients, instructions, category, cuisine, cook_time, servings, difficulty, video_url } = req.body;
    ...
    const result = await pool.query(
      ...
      [req.user.id, title, description, ingredients, instructions, category, cuisine, cook_time, servings, difficulty, thumbnailPath, video_url, videoFilePath]
    );
    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get('/api/recipes', async (req, res) => {
  try {
    ...
    res.json({ recipes: result.rows, total: parseInt(countResult.rows[0].count) });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.put('/api/recipes/:id', authMiddleware, recipeUpload, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [title, description, ingredients, instructions, category, cuisine, cook_time, servings, difficulty, thumbnailPath, video_url, videoFilePath, req.params.id]
    );
    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.post('/api/recipes/:id/like', authMiddleware, async (req, res) => {
  try {
    const existing = await pool.query('SELECT * FROM recipe_likes WHERE recipe_id=$1 AND user_id=$2', [req.params.id, req.user.id]);
    if (existing.rows.length > 0) {
      await pool.query('DELETE FROM recipe_likes WHERE recipe_id=$1 AND user_id=$2', [req.params.id, req.user.id]);
      await pool.query('UPDATE recipes SET likes = likes - 1 WHERE id = $1', [req.params.id]);
      return res.json({ liked: false });
    }
    await pool.query('INSERT INTO recipe_likes (recipe_id, user_id) VALUES ($1, $2)', [req.params.id, req.user.id]);
    await pool.query('UPDATE recipes SET likes = likes + 1 WHERE id = $1', [req.params.id]);
    res.json({ liked: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get('/api/dashboard/stats', authMiddleware, async (req, res) => {
  try {
    ...
    const categoryDist = await pool.query(
      'SELECT category, COUNT(*) as count FROM recipes WHERE user_id = $1 AND category IS NOT NULL GROUP BY category', [userId]
    );
...
app.get('/api/admin/stats', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const totalViews = await pool.query('SELECT COALESCE(SUM(views), 0) as total FROM recipes');
    const totalLikes = await pool.query('SELECT COALESCE(SUM(likes), 0) as total FROM recipes');
...
app.get('/api/admin/users', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const result = await pool.query('SELECT id, name, email, mobile, gender, age, profile_photo, role, created_at FROM users ORDER BY created_at DESC');
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/my-recipes', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM recipes WHERE user_id = $1 ORDER BY created_at DESC', [req.user.id]);
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
function getMockYouTubeVideos(query) {
  const videos = [
    { id: { videoId: 'aqz-KE-bpKQ' }, snippet: { title: `Signature ${query} Masterclass`, description: 'Step-by-step professional cooking tutorial for the perfect dish.', thumbnails: { high: { url: 'https://img.youtube.com/vi/aqz-KE-bpKQ/hqdefault.jpg' } }, channelTitle: 'Culinary Arts' } },
    { id: { videoId: 'fAnvTfSTGQA' }, snippet: { title: `Traditional ${query} | Home Cooking`, description: 'Classic family recipe with easy-to-follow instructions.', thumbnails: { high: { url: 'https://img.youtube.com/vi/fAnvTfSTGQA/hqdefault.jpg' } }, channelTitle: 'Home Kitchen' } },
    { id: { videoId: '9_7mY5p_tYg' }, snippet: { title: `Gourmet ${query} Recipe Guide`, description: 'elevate your cooking with these professional techniques.', thumbnails: { high: { url: 'https://img.youtube.com/vi/9_7mY5p_tYg/hqdefault.jpg' } }, channelTitle: 'Chef Life' } },
    { id: { videoId: 'M9-S6S-k6kE' }, snippet: { title: `Quick & Healthy ${query}`, description: 'Nutritious and delicious meal prep ideas.', thumbnails: { high: { url: 'https://img.youtube.com/vi/M9-S6S-k6kE/hqdefault.jpg' } }, channelTitle: 'Healthy Eats' } },
    { id: { videoId: '_XW6ToYdf0s' }, snippet: { title: `Ultimate ${query} Preparation`, description: 'The absolute best way to prepare this classic favorite.', thumbnails: { high: { url: 'https://img.youtube.com/vi/_XW6ToYdf0s/hqdefault.jpg' } }, channelTitle: 'Master Chef' } },
    { id: { videoId: 'y_f0OaVfD0U' }, snippet: { title: `Homemade ${query} from Scratch`, description: 'Everything from ingredients to the final plate.', thumbnails: { high: { url: 'https://img.youtube.com/vi/y_f0OaVfD0U/hqdefault.jpg' } }, channelTitle: 'Basics of Cooking' } },
  ];
  return videos;
}
